...
ALTER TABLE public.filmactor OWNER TO postgres;
...
ALTER TABLE public.filmcategory OWNER TO postgres;
...
CREATE VIEW public.actor_info AS
 SELECT a.actor_id,
    ...
    public.group_concat(DISTINCT (((c.name)::text || ': '::text) || ( SELECT public.group_concat((f.title)::text) AS group_concat
           FROM ((public.film f
             JOIN public.filmcategory fc_1 ON ((f.film_id = fc_1.film_id)))
             JOIN public.filmactor fa_1 ON ((f.film_id = fa_1.film_id)))
          WHERE ((fc_1.category_id = c.category_id) AND (fa_1.actor_id = a.actor_id))
          GROUP BY fa_1.actor_id))) AS film_info
   FROM (((public.actor a
     LEFT JOIN public.filmactor fa ON ((a.actor_id = fa.actor_id)))
     LEFT JOIN public.filmcategory fc ON ((fa.film_id = fc.film_id)))
...
CREATE VIEW public.film_list AS
 SELECT film.film_id AS fid,
   ...
   FROM ((((public.category
     LEFT JOIN public.filmcategory ON ((category.category_id = filmcategory.category_id)))
     LEFT JOIN public.film ON ((filmcategory.film_id = film.film_id)))
     JOIN public.filmactor ON ((film.film_id = filmactor.film_id)))
     JOIN public.actor ON ((filmactor.actor_id = actor.actor_id)))
...
CREATE VIEW public.sales_by_film_category AS
 SELECT c.name AS category,
   ...
   FROM (((((public.payment p
     ...
     JOIN public.filmcategory fc ON ((f.film_id = fc.film_id)))
...
ALTER TABLE ONLY public.filmactor
...
CREATE INDEX idx_fk_film_id ON public.filmactor USING btree (film_id);
...
CREATE TRIGGER last_updated BEFORE UPDATE ON public.filmactor FOR EACH ROW EXECUTE FUNCTION public.last_updated();
...
CREATE TRIGGER last_updated BEFORE UPDATE ON public.filmcategory FOR EACH ROW EXECUTE FUNCTION public.last_updated();
